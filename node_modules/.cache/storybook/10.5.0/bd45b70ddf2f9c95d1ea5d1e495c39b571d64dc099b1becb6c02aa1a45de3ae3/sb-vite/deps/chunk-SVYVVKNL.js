...
var inferControl = (argType, name, matchers) => {
  ...
  if (type) {
    if (matchers.color && matchers.color.test(name)) {
      ...
      controlType !== "enum" && import_client_logger.logger.warn(
...
var inferType = (value, name, visited, cache) => {
  ...
  if (value) {
    ...
    if (visited.has(value))
      return import_client_logger2.logger.warn(dedent`
...
function splitCR(source) {
  let matches2 = source.match(/\r+$/);
  return matches2 == null ? ["", source] : [source.slice(-matches2[0].length), source.slice(0, -matches2[0].length)];
}
function splitSpace(source) {
  let matches2 = source.match(/^\s+/);
  return matches2 == null ? ["", source] : [source.slice(0, matches2[0].length), source.slice(matches2[0].length)];
}
...
function shouldSkipStoryDocsEmit(parameters) {
  ...
  return parameters?.__isPortableStory ? true : sourceParams?.type === "dynamic" ? false : !isArgsStory || sourceParams?.code !== void 0 || sourceParams?.type === "code";
}
function shouldWaitForServiceSnippet(parameters, storyPrepared) {
  return globalThis.FEATURES?.experimentalDocgenServer ? !storyPrepared || !shouldSkipStoryDocsEmit(parameters) : false;
}
...
//# sourceMappingURL=chunk-SVYVVKNL.js.map
