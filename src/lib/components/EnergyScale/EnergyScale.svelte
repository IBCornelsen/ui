...
<div class="w-full select-none">
	...
	<div class="relative flex h-8 w-full overflow-hidden rounded-md">
		{#each segments as segment, index (segment.label)}
			...
			>
				<span class="text-[11px] font-bold text-neutral-900">{segment.label}</span>
			</div>
		{/each}
		...
		{#each markers as marker (marker.value + (marker.position ?? "top"))}
			...
			>
				{#if marker.position === "bottom"}
					<CaretUpIcon size={24} weight="fill" class="translate-y-[9px] text-white" />
				{:else}
					<CaretDownIcon size={24} weight="fill" class="-translate-y-[9px] text-white" />
				{/if}
			</div>
		{/each}
	</div>
