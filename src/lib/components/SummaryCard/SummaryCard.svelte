<script lang="ts">
	...
	function onCardTouchStart(event: TouchEvent) {
		...
		dragging = !expanded;
		if (dragging && panelEl) {
			contentHeight = (panelEl.firstElementChild as HTMLElement | null)?.scrollHeight ?? 0;
...
>
	...
	{#if scale}
		<div class="flex flex-col gap-2 border-neutral-100 lg:border-b lg:pb-4">
			<div class="hidden items-center gap-1.5 lg:flex">
				...
				<Tooltip text="Die Einschätzung der Effizienz erfolgt nach Eingabe der Verbrauchsdaten.">
					<button
						type="button"
	...
	<div class="collapsible" class:open={expanded} bind:this={panelEl}>
		<div class="collapsible-inner flex flex-col gap-4">
			...
			<div class="flex flex-col gap-2 border-b border-neutral-100 pb-4 lg:order-first">
             ...
             Titel = h5-Typografie in primary-700, Preis orange, MwSt.-Zusatz grau. -->
				<div class="flex items-center justify-between gap-3">
					<span class="text-lg leading-snug font-bold text-primary-700">{produkt}</span>
					<div class="flex shrink-0 items-baseline gap-1.5">
						<span class="text-base font-bold whitespace-nowrap text-secondary-600">{preis} €</span>
									...
									{#if step.messages.length > 0}
										<ul class="mt-2 flex flex-col gap-2">
											{#each step.messages as message (message.id)}
												...
												>
													<span class="flex-1 text-sm">{@html message.html}</span>
...
{#snippet stepHeading(step: StepInfo)}
	<span
		...
		class:text-primary-700={step.severity === "none" && step.state === "current"}
		class:text-primary-900={step.severity === "none" && step.state === "done"}
		class:text-neutral-600={step.severity === "none" && step.state === "upcoming"}
	>
		{step.label}
	</span>
{/snippet}
